...
public class Application extends JFrame {
    ...
    private void initialize() {
        ...
        twitterSource = new PlaybackTwitterSource(1.0, new ObjectSource("TwitterMapperStarter/data/TwitterCapture.jobj"));
...
//                map().setToolTipText("This is a tooltip");
                ...
                for (MapMarker m : markers) {
                    if (m instanceof MapMarkerComplex) {
                        BufferedImage profileImage = ((MapMarkerComplex) m).getImage();
                        File outputfile = new File("image.png");
                        try {
                            String toolTipText = ((MapMarkerComplex) m).getStatus().getText();
                            ImageIO.write(profileImage, "png", outputfile);
                            StringBuilder sb = new StringBuilder(128);
                            sb.append("<html><img src='");
                            sb.append(outputfile.toURI().toURL());
                            sb.append("'width=35 height=35><span>" + toolTipText + "</span></html>");
                            map().setToolTipText(sb.toString());
    ...
    public void terminateQuery(Query query) {
        ...
        if(query.getCheckBox().equals(false)) {
            queries.remove(query);
            Set<String> allterms = getQueryTerms();
            allterms.remove(query);
            twitterSource.setFilterTerms(allterms);
